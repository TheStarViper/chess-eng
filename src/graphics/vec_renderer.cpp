...
namespace VectorRenderer {
    ...
    void DrawOvergrownVines(int boardX, int boardY, float boardSize, int id) {
        ...
        static int lastTrackedHoveredId = -1;
        static float hoverTransitionStartTime = 0.0f;

        if (hoveredLeafThisFrame != lastTrackedHoveredId) {
            lastTrackedHoveredId = hoveredLeafThisFrame;
            hoverTransitionStartTime = GetTime();
        }

        float timeElapsed = GetTime() - hoverTransitionStartTime;
        float duration = 0.2f; 
        float progress = timeElapsed / duration;
        if (progress > 1.0f) progress = 1.0f;

        float easedT = Easings::EaseInOutCubic(progress);
        ...
        auto UpdateAndDrawLeaf = [&](float x, float y, float size, float angleDegrees) {
            ...
            float currentLeafEasedT = 0.0f;
            if (hoveredLeafThisFrame == id) {
                currentLeafEasedT = easedT; 
            } else if (lastTrackedHoveredId == id) {
                currentLeafEasedT = 1.0f - easedT;
            }

            float finalSize = size + (size * 0.30f) * currentLeafEasedT;
